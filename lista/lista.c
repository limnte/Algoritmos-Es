...
Tarefa CriarTarefa(char * Titulo, char * Responsavel, int Status, float Progresso, int Avaliacao){
    ...
    strcpy(T.Responsavel, Responsavel);
...
Lista * CriarLista (int C){
    ...
    printf("A lista foi criada e armazenada no endereço %X.\n", L);
    ...
    if(L->Dados == NULL){
        printf("ERRO: NÃO FOI POSSIVEL ALOCAR MEMORIA PARA O VETOR DE TAREFAS\n");
        free(L);
        return NULL;
    }
...
void RemoverTarefa(Lista * L, int Indice){
    ...
    if(Indice < 0) return;
    if(Indice >= L->Tamanho) return;

    for(int i = Indice; i < L->Tamanho; i++){
